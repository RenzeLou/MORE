...
class dataloader():
    def __init__(self, data_file, wordvec_file=None, rel2id_file=None, similarity_file=None, same_level_pair_file=None,
                 max_len=120, random_init=False, seed=None, data_type='fewrel'):
        ...
        with open(data_file, 'r') as r:
            data = json.load(r)  # 我们模型需要的json格式
        ...
        for i in range(len(data)):
            if i not in pop_list:
                temp_data.append(copy.deepcopy(data[i]))
    ...
    def data_to_padded_idx_data(self, data, max_len=120):
        ...
        padded_idx_data[2] = np.append(np.array(data['sentence'][:max_len]), np.array([self.BLANK] * padnum))
    ...
    def next_batch_cluster(self, batch_size=128, class_num_ratio=0.25, random_shuffle=True, inclass_augment=False):
        ...
        # print("max_num:",max_class_num)
        # print("-----class_list_size-----")
        # print(len(class_list))
        self.batch_cluster_label = copy.deepcopy(class_list)
        class_list = class_list * (batch_size // class_num + 1)
        if inclass_augment:
            class_list = class_list * 2
        # print("-----class_list_size-----")
        # print(len(class_list))
        num = batch_size * 2 if inclass_augment else batch_size
        for i, index in enumerate(class_list):
            if i >= num:
                break
            while True:
                ...
                if have_chose.get(instance_index, 0) == 0 or rel_chose_num[index] >= len(
                    ...
                    try:
                        ...
                        if len(self.processed_data[instance_index]['sentence']) > self.max_len:
                            print("长度")
                            continue
                        else:
        ...
        return batch_data, batch_label, self.batch_cluster_label
    ...
    def _data_(self, each_class_num=None):
        data_relid = []
        data_to_cluster = []
        if each_class_num is None:
            for item in self.processed_data:
            ...
            for item in self.processed_data:
                if reltype_counter.get(item['relid'], 0) >= each_class_num:
                    continue
                else:
                    data_to_cluster.append(self.data_to_padded_idx_data(item))
                    data_relid.append(item['relid'])
...
if __name__ == '__main__':
    ...
    batch_data, batch_label, label_list = dataloader_test.next_batch_cluster(128, 0.25)

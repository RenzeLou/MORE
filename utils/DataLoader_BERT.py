...
'''
The original file come from https://github.com/thunlp/RSN/tree/master/lib
And we modified it for our use
'''


class dataloader_BERT():
    def __init__(self, data_file, wordvec_file=None, rel2id_file=None, similarity_file=None, same_level_pair_file=None,
                 max_len=120, random_init=False, seed=None):
        if not random_init and seed:
            tl.seed_torch(seed)
        if not wordvec_file:
            wordvec_file = '../data/wordvec/word_vec.json'

        self.max_len = 120  # max_len Using bert input shape, 128 sequence size
        ...
        for item in ori_data:
            ...
            item['sentence'] = list(filter(None, item['sentence']))
            # ori_sentence = deepcopy(data[i]['sentence'])
            # sentence-as-str to sentence-as-index
            e1_begin = item['head']['e1_begin']
            e1_end = item['head']['e1_end']
            e2_begin = item['tail']['e2_begin']
            e2_end = item['tail']['e2_end']
            if e1_begin > e2_begin:
                item['sentence'].insert(e1_begin, '<e1>')
                item['sentence'].insert(e1_end + 2, '</e1>')
                item['sentence'].insert(e2_begin, '<e2>')
                item['sentence'].insert(e2_end + 2, '</e2>')
            else:
                item['sentence'].insert(e2_begin, '<e2>')
                item['sentence'].insert(e2_end + 2, '</e2>')
                item['sentence'].insert(e1_begin, '<e1>')
                item['sentence'].insert(e1_end + 2, '</e1>')
            item['sentence'] = ' '.join(item['sentence'])
        ...
        for i, item in enumerate(data):
            temp_item = {}
            # tokenized sentence [114042, 1605, 6737, 10631, 18251, 39, 114042, 38]
            temp_item['ori_sentence'] = ori_data[i]['sentence']
    ...
    def data_to_padded_idx_data_bert(self, data, max_len=120):
        '''padded_idx_data as [pos1array, pos2array, sentence]'''
        padded_idx_data = data['ori_sentence']
        return padded_idx_data

    def sequential_sampling_rel(self, all_relation, chose_relation, class_num):
        class_list = []
        candidate_relation = np.setdiff1d(all_relation, chose_relation, True).tolist()
        if len(candidate_relation) < class_num:
            if len(candidate_relation) != 0:
                class_list = copy.deepcopy(candidate_relation)
                class_num -= len(candidate_relation)
            chose_relation = []
            candidate_relation = all_relation

        temp = random.sample(np.setdiff1d(candidate_relation, class_list, True).tolist(), class_num)

        if len(class_list) == 0:
            class_list += temp
            chose_relation += class_list
        else:
            chose_relation = copy.deepcopy(temp)
            class_list += temp

        return class_list, chose_relation

    def next_batch_noRandom(self, batch_size=128, class_num_ratio=0.25, random_shuffle=True, each_relation_num=dict(),
                            chose_relation=[],
                            chose_sample=[]):
        batch_data = []
        have_chose = dict()
        max_class_num = len(self.lists_for_single_rel_mention.keys())
        class_num = int(batch_size * class_num_ratio)
        all_relation = list(self.lists_for_single_rel_mention.keys())
        delete_relation = []
        for k, v in each_relation_num.items():
            if v < math.ceil(batch_size / class_num):
                delete_relation.append(k)
        all_relation = np.setdiff1d(all_relation, delete_relation, True).tolist()

        # if len(all_relation) < class_num:
        #     return None, None, None, None, None, None
        # else:
        #     class_list, chose_relation = self.sequential_sampling_rel(all_relation, chose_relation, class_num)
        try:
            class_list, chose_relation = self.sequential_sampling_rel(all_relation, chose_relation, class_num)
        except:
            if len(all_relation) < class_num:
                ...
                class_list = random.sample(all_relation, class_num)
                chose_relation = []

        self.batch_cluster_label = class_list

        random.shuffle(class_list)

        class_list = class_list * (batch_size // class_num + 1)
        for i, index in enumerate(class_list):
            if i >= batch_size:
                break
            while True:
                instance_index = random.choice(self.lists_for_single_rel_mention[index])
                if have_chose.get(instance_index, 0) == 0 and (instance_index not in chose_sample):
                    chose_sample.append(instance_index)
                    batch_data.append(self.data_to_padded_idx_data(self.processed_data[instance_index]))
                    have_chose[instance_index] = 1
                    each_relation_num[index] -= 1
                    break

        batch_label = class_list[:batch_size]

        if random_shuffle:
            batch_data, batch_label = tl.shuffle2list(batch_data, batch_label)

        batch_data = torch.from_numpy(np.array(batch_data, dtype=np.int64))
        batch_label = torch.from_numpy(np.array(batch_label, dtype=np.int64))

        return batch_data, batch_label, chose_relation, chose_sample, each_relation_num, self.batch_cluster_label

    def next_batch_cluster(self, batch_size=128, class_num_ratio=0.25, random_shuffle=True, inclass_augment=False):
        batch_data = []
        batch_sentence = []
        have_chose = dict()
        max_class_num = len(self.lists_for_single_rel_mention.keys())
        ...
        try:
            ...
            if max_class_num < class_num:
                print("class_num_ratio is too large,class num reduce to {}".format(max_class_num))
                class_list = random.sample(list(self.lists_for_single_rel_mention.keys()), max_class_num)
                class_num = max_class_num
            elif class_num <= 0:
                print("class_num_ratio <= 0")
            else:
        ...
        for i, index in enumerate(class_list):
            ...
            while True:
                instance_index = random.choice(self.lists_for_single_rel_mention[index])
                if have_chose.get(instance_index, 0) == 0:
                    # append entities information batch_index
                    batch_data.append(self.data_to_padded_idx_data(self.processed_data[instance_index]))
                    batch_sentence.append(self.processed_data[instance_index]['ori_sentence'])
                    have_chose[instance_index] = 1
                    break
        ...
        return batch_data, batch_label, batch_sentence, self.batch_cluster_label

    # batching functions
    def next_batch_same(self, batch_size):  # return a list with numpy.array[dtype=int32]
    ...
    def _data_(self):
        ...
        data_to_cluster = []
        for item in self.processed_data:
            data_to_cluster.append(self.data_to_padded_idx_data_bert(item))
...
if __name__ == '__main__':
    ''' test the dataloader'''
    # dataloader_test = dataloader('../data/datasets/fewrel_ori/fewrel80_test_train.json',
    #                              '../data/wordvec/word_vec.json')
    # batch_data, batch_label, _, label_list = dataloader_test.next_batch_cluster(128, 0.25)
    # print("labbel_list:", label_list)
    # print("batch_data:", batch_data)
    # print("batch_label:", batch_label)
    # print("batch_data.size:", batch_data.shape)
    # print("batch_label.size:", batch_label.shape)
    # print(label_list)
    # print("unique label:", len(label_list))
    ...
    # for i in range(500):
    #     dataloader_train.next_batch(opt.batch_size,opt.same_ratio)
    #     # dataloader_train.next_batch_cluster(opt.batch_size, opt.class_num_ratio, True)
    # end_time = time.time()

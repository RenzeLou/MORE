...
from modules import RSN
from modules.Cluster import K_means, mean_shift
...
def k_means_cluster_evaluation(model: BasicModel, opt: config.Option, test_data, test_data_label, loger):
    ...
    with torch.no_grad():
        cluster_result, cluster_msg, cluster_center, features = K_means(test_data, model.pred_vector,
...
def process_and_train_FL(model: BasicModel, opt: config.Option):
                ...
                if opt.early_stop is not None:
                    ...
                    if early_stop_record == opt.early_stop:
                        print("~~~~~~~~~ The loss can't be reduced in {} step, early stop! ~~~~~~~~~~~~".format(
                            opt.early_stop * 100))
                        best_test_new, best_test_b3, NMI_score = k_means_cluster_evaluation(model, opt, test_data,
                                                                                            test_data_label, loger)
                ...
                with torch.no_grad():
                    ...
                    if opt.dataset.startswith("fewrel"):
                        ...
                        for iterion in range(opt.eval_num):
                            ...
                            cluster_result, cluster_msg, cluster_center, features = K_means(val_data, model.pred_vector,
                                                                                            K_num, opt)
                        ...
                        cluster_result, cluster_msg, cluster_center, features = mean_shift(val_data, model.pred_vector,
                                                                                           opt)
                        cluster_eval_b3 = ClusterEvaluation(val_data_label, cluster_result).printEvaluation(
                    ...
                    if opt.record_test:
                        test_result, test_msg, test_center, test_features = K_means(test_data, model.pred_vector,
                                                                                    len(np.unique(test_data_label)),
                                                                                    opt)
                        cluster_test_b3 = ClusterEvaluation(test_data_label, cluster_result).printEvaluation(
                            print_flag=print_flag, extra_info=True)
                        NMI_score_test = normalized_mutual_info_score(test_data_label, cluster_result)
                        msger_test.record_message(
                            [global_step, opt.batch_size[batch_size_chose], opt.lr[opt.lr_chose], NMI_score_test,
                             cluster_test_b3['F1'], cluster_test_b3['precision'],
                             cluster_test_b3['recall'], test_msg])
    ...
    if opt.dataset.startswith("fewrel"):
        ...
        cluster_result_ms, cluster_msg_ms,_,_ = mean_shift(test_data, model.pred_vector, opt)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--class_ratio", type=float, default=0.5)
    parser.add_argument("--squared", type=int, default=0)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--early_stop", type=int, default=None)
    parser.add_argument("--record_test", type=int, default=0)
    ...
    print("use cuda:", os.environ["CUDA_VISIBLE_DEVICES"])

    # initialize option
    opt.train_loss_type = args.train_loss_type
    opt.testset_loss_type = args.testset_loss_type

    opt.load_model_name = args.load_model_name  # e.g. 'RSN_FL_best.pt'
    ...
    rsn_fl = RSN.RSN_FL(random_init, opt)
    ...
    process_and_train_FL(rsn_fl, opt)
    end_time = time.time()

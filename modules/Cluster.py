...
def model_pred_BERT(datasets, pred_vector, labels, opt):
    vectors = pred_vector(datasets, labels, opt)
    try:
        vectors = vectors.detach().cpu().numpy().tolist()
    except:
        vectors = vectors.tolist()
    feature_embeddings = np.array(vectors)

    # return embedding of one batch input
    return feature_embeddings


def mean_shift_BERT(datasets, pred_vector, labels, opt):

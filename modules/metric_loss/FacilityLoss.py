...
class Facility(object):
    ...
    def _find_loss_augmented_facility_idx(self, pairwise_distances, labels, chosen_ids,
        ...
        if len(chosen_ids) == 0:
            ...
            temp = torch.min(-torch.cat((pairwise_distances_chosen_tile, pairwise_distances_candidate.reshape(1, -1)), 0),0)[0]
        # [num_chose, num_candicates * batch]
        # print("pairwise_distances_chosen.size:", pairwise_distances_chosen.shape)
        # print("pairwise_distances_candidate.size:",  pairwise_distances_candidate.shape)
        # print("pairwise_distances_chosen_tile.size:",pairwise_distances_chosen_tile.shape)
        # print("pairwise_distances_candidate.reshape(1, -1).size:",pairwise_distances_candidate.reshape(1, -1).shape)


        # temp:[num_candidate * embedding] 1D Tensor
        candidate_scores = -1.0 * torch.sum(temp.reshape(num_candidates, -1), 1)
        # candidate_scores = [num_candicates,] 1D Tensor
        # candidate_scores = -1.0 * np.sum(
        #     np.min(-np.row_stack((pairwise_distances_chosen_tile,
        #                           pairwise_distances_candidate.reshape(1, -1)  # [1, num_candicates * batch]
        #                           )), 0).reshape(num_candidates, -1)  # [num_candicates, batch]
        #     , axis=1)
        # nmi_scores = torch.from_numpy(np.zeros([num_candidates]))
        # for i in range(num_candidates):
        #     # 每次从当前candidate中取出一个作为新的簇心，计算margin
        #     lis = chosen_ids.tolist()
        #     lis.append(candidate_ids[i])
        #     lis = torch.from_numpy(np.array(lis))
        #     predictions = self.get_cluster_assignment(pairwise_distances, lis)
        #     nmi_score_i = self.compute_clustering_score(labels, predictions, margin_type)
        #     # return 1 - NMI score as the structured loss.
        #     # because NMI is higher the better [0,1].
        #     nmi_scores[i] = nmi_score_i*margin_multiplier

        # print("candidate_scores.type:",type(candidate_scores))
        # print("nmi_scores.type:", type(nmi_scores))
    ...
    def compute_augmented_facility_locations(self, pairwise_distances, labels,
        ...
        # 找num_classes个簇心点集
        for i in range(num_classes):
            # 每次都在候选的candidate里面找一个loss agumented最大的新的簇心，找满num_classes个为止，gready search
    ...
    def update_medoid_per_cluster(self, pairwise_distances, pairwise_distances_subset,
        ...
        # print("---score_fac.shape:",scores_fac.shape)

        # for i in range(num_candidates):
        #     # swap the current medoid with the candidate cluster member
        #     candidate_medoid = cluster_member_ids[i]
    ...
    def update_all_medoids(self, pairwise_distances, predictions, labels, chosen_ids,
        ...
        for i in range(num_classes):
            ...
            this_cluster_ids = torch.where(mask)[0]
            # print("this_cluster_ids:",this_cluster_ids)
            # print("mask:",mask)
            # print("i:",i)
            temp = (tl.gather(pairwise_distances, this_cluster_ids)).T
            # print("this_cluster_ids.size:", this_cluster_ids.shape)
            # print("this_cluster_ids:",this_cluster_ids)
    ...
    def compute(self,pairwise_distances ,labels ,opt: Option, global_step):
        ...
        chosen_ids = self.compute_augmented_facility_locations(pairwise_distances, labels, all_ids, margin_multiplier,
                                                               margin_type)  # time consume!!!!!!!!! 8～9 s
        ...
        # s4 = time.time()
        # print("chose id: ", s4 - s3, " seconds")

        # s1 = time.time()
        score_pred = self.compute_facility_energy(pairwise_distances, chosen_ids)
        # s2 = time.time()
        # print("compute facility score:", s2 - s1)

        # Given the predicted centroids, compute the cluster assignments.
        # s3 = time.time()
        predictions = self.get_cluster_assignment(pairwise_distances, chosen_ids)
        # s4 = time.time()
        # print("cluster assignment:", s4 - s3, "seconds")
        ...
        # s2 = time.time()
        # print("score_gt:", s2 - s1, " seconds")
        # print("score_gt:",score_gt)
        # s2 = time.time()
        # print("score_gt:",s2-s1," seconds")
        # print("score_gt:",score_gt)
        # try:
        #     score_gt.backward(retain_graph=True)
        # except:
        #     print("autograd4 failed")
        # exit()
        # score_gt.backward()
